...
我们可以更改一下我们的线程池，使得它支持调用者指定`BlockingQueue`的具体类型：
```Java
public MyThreadPoolExecutor(int coreSize, int maxSize, long keepAliveTime,
...
```
由于在构造方法中，我们只能根据用户指定的具体阻塞队列类型来创建线程池，所以我们不能提前指定大小，这一步交给调用者自己去做：

```Java
new ArrayBlockingQueue<>(50); // 传入一个容量为50的ArrayBlockingQueue
```
到此，我们的线程池就全部结束了。最终的全部代码如下：
```Java
@Slf4j
public class MyThreadPoolExecutor {
    private final int coreSize; // 核心线程数量
